...
//
// for new Teensy 24bit 2ch (4ch) brain-duino with filter frequency control LED
...
// ledPin2 -->> 7
// ledPin7 -->> 8 but not use this case
//
...
//
// this 2 or 4 need to setup
// // 2 = 2 chanel system, 4 = 4 channel system
//
//
int Nchannel=2;
//
...
//    myTimer.begin(callback, 20);  // not working
//    myTimer.begin(callback, 21);  // blinkLED to run 47K Hz
  myTimer.begin(callback, 25);  // blinkLED to run 39965.625 Hz
//    myTimer.begin(callback, 30);  // blinkLED to run 33419.53125 Hz
//    myTimer.begin(callback, 35);  // blinkLED to run 28596.09375 Hz
//    myTimer.begin(callback, 40);  // blinkLED to run 25150.78125 Hz
//    myTimer.begin(callback, 50);  // blinkLED to run 20000 Hz
//    myTimer.begin(callback, 320);  // blinkLED to run 3123.81836 Hz
      myTimer.priority(0);
//    myTimer.begin(callback, 50);  // blinkLED to run 19982.8125 Hz

/*
//    myTimer1.begin(callback1, 192); // 200 : 5K Hz sampling
//    myTimer1.begin(callback1, 482); // 482 : 2074 Hz sampling
      myTimer1.begin(callback1, 965); // 965 : 1024 Hz sampling
//    myTimer1.begin(callback1, 1930); // 1930 : 512Hz sampling

      myTimer1.priority(100);
...
// setup 12Hz Sin wave for impedance check 
//
...
//
  ...
  // 2048576 -> clip max 5mV p-p : input 25µV, 1mV : 5µV : gain X0.01, OSC ON : 6.5mV clip
  // 1048576 -> clip max 2.5mV p-p : input 25µV, 0.5mV : 5µV : gain X0.05, OSC ON : 3.3mV clip
  // 65536 -> clip max 200µV p-p : input 30µV, 30µV : 5µV : gain X1, OSC ON : 200µV clip
  // 20480 -> clip max 50µV p-p : input 45µV, 6µV : 5µV : gain X1, OSC ON : 60µV clip
  //
...
void setupMyTimer (int speedA) {
  myTimer.end(); 
  myTimer.begin(callback, speedA);  // // setup high cut freqeuncy
  myTimer.priority(0);
...
void setupMyTimer1 (int speedA) {
  ledON2=0;
  myTimer1.end(); 
  myTimer1.begin(callbackX, speedA);  //  LED blink speed show high cut filter frequency setup and/or direct : OFF LED
...
void  sendData(int NchannelX) {
  ...
  // 24 bit 5 ch
  if ( NchannelX == 5 ) {
    HEXprint24bit5ch( value0[0], value0[1],  value0[2], value1[0], value1[1],  value1[2], value2[0], value2[1],  value2[2], value3[0], value3[1],  value3[2], value4[0], value4[1],  value4[2] );

  }

  // 24 bit 4 ch
  else if ( NchannelX == 4 ) {
    HEXprint24bit4ch( value0[0], value0[1],  value0[2], value1[0], value1[1],  value1[2], value2[0], value2[1],  value2[2], value3[0], value3[1],  value3[2] );
...
void HEXprint24bit(byte inVal1, byte inVal2, byte inVal3, byte inVal4, byte inVal5,  byte inVal6)  {
  ...
  if ( inVal2 >= 16 ) {
    HWSERIAL.print( inVal2/16, HEX );
    
    HWSERIAL.write(0x09);
    HWSERIAL.print( inVal2-((inVal2/16)*16), HEX );
  }
  else {
     HWSERIAL.print( 0 );
...
void HEXprint24bit4ch(byte inVal1, byte inVal2, byte inVal3, byte inVal4, byte inVal5,  byte inVal6, byte inVal7, byte inVal8, byte inVal9, byte inVal10, byte inVal11,  byte inVal12)  {
  byte outStream[16];
...
void HEXprint24bit5ch(byte inVal1, byte inVal2, byte inVal3, byte inVal4, byte inVal5,  byte inVal6, byte inVal7, byte inVal8, byte inVal9, byte inVal10, byte inVal11,  byte inVal12, byte inVal13, byte inVal14,  byte inVal15 )  {
  byte outStream[16];
...
// B is problem (-CH1 ) -->> T
     ...
     if (  (inChar == 'S') || (inChar == 'I') ||  (inChar == 'P') || (inChar == 'O') 
     || (inChar == 'Q') || (inChar == 'Z') || (inChar == 'M') || (inChar == 'V') || (inChar == 'W')  
     || (inChar == 'A') || (inChar == 'T') || (inChar == 'C') || (inChar == 'D') || (inChar == 'E') 
     || (inChar == 'F') || (inChar == 'G') || (inChar == 'H') || (inChar == 'J') || (inChar == 'K')
     || (inChar == 'X') || (inChar == 'Y') || (inChar == 'U') ) 
     {
      stringComplete = true;
