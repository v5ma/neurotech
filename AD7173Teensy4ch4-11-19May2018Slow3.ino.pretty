...
// NeurofoxCursa
//
// here ... note questions about calibration procedure, IBVA functions
...
setup ()
{
  ...
  myTimer.begin (callback, 25);	// re filter clock ... blinkLED to run 39965.625 Hz
  myTimer.priority (0);
...
setupAD7173 ()
{
  ...
  AD7173.init ();
  AD7173.sync (); // ??? not defined ? use resync() instead ?
...
// setup for 2channel 24bit
//
  if (Nchannel == 2)
    {
      if (InputX == 1) // raw, not filtered
	{
	  ...
	  AD7173.set_offset_config (OFFSET2, CH2offset);  // different offset, since no filter
	  AD7173.set_offset_config (OFFSET3, CH3offset);
	}
      else if (InputX == 0) // here ... selects pinout to use with filter
	{
	  AD7173.set_channel_config (CH0, true, SETUP0, AIN4, AIN5);
	  AD7173.set_channel_config (CH1, true, SETUP1, AIN6, AIN7);

	  AD7173.set_setup_config (SETUP0, BIPOLAR);  // note bipolar allows potentially 10 v difference
	  AD7173.set_setup_config (SETUP1, BIPOLAR);

	  AD7173.set_offset_config (OFFSET0, CH0offset);  // dc offset, different for filter
...
//
  ...
  // what's up with SPS_503 ... indicates 503 hz sampling per channel - 2 channels yields 251.5
...
sendData (int NchannelX)
{
  ...
  // 24 bit 2ch to 12 bit 4 ch
    ...
    {
      HEXprint24bit (value0[0], value0[1], value0[2], value1[0], value1[1],
		     value1[2]);
...
void
HEXprint24bit (byte inVal1, byte inVal2, byte inVal3, byte inVal4,
	       byte inVal5, byte inVal6)
{
...
void
HEXprint24bit4ch (byte inVal1, byte inVal2, byte inVal3, byte inVal4,
...
void
HEXprint24bit5ch (byte inVal1, byte inVal2, byte inVal3, byte inVal4,
		  byte inVal5, byte inVal6, byte inVal7, byte inVal8,
		  byte inVal9, byte inVal10, byte inVal11, byte inVal12,
